...
export default function EmployeesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(r => [r.full_name, r.designation, r.dept_name, r.branch_name, r.employment_type, r.status, r.manager_name].some(v => v && String(v).toLowerCase().includes(q)));
  }, [data, search]);

  const openAdd = () => { setForm(emptyEmp); setFormError(null); setModal('add'); };
  ...
  return (
    <>
      ...
      <div className="relative mb-4">
        ...
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, designation, department, branch, status..." className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
      </div>
